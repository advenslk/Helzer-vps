...
class VPSCog(commands.Cog):
    ...
    async def list_vps(self, interaction: discord.Interaction) -> None:
        ...
        if not items:
            await interaction.response.send_message(f"{E.INFO} You don't have any VPS yet.", ephemeral=True)
            return
        text = "\n".join(
            f"**HX-{v['id']}** · `{v['name']}` · {E.RUNNING if v['status'] == 'running' else E.STOPPED} `{v['status']}`"
            for v in items
        )
        await interaction.response.send_message(view=discord.ui.LayoutView())
        await interaction.edit_original_response(content=f"# {E.VPS} Your VPS\n{text}")
    ...
    async def manage_vps(self, interaction: discord.Interaction, vps_id: int) -> None:
        ...
        if not vps or vps["owner_id"] != interaction.user.id:
            ...
            return
        await interaction.response.send_message(view=VPSPanel(vps, self.bot.service))
    ...
    async def admin_ai(self, interaction: discord.Interaction, prompt: str) -> None:
        ...
        try:
            ...
            name = f"Helzer-{request.target_user_id}"
            vps = await self.bot.service.create_vps(
                request.target_user_id,
                VPSSpec(name=name, cpu_cores=request.cpu_cores, ram_mb=request.ram_mb, disk_gb=request.disk_gb),
            )
            try:
                await member.send(
    ...
    @admin.command(name="vps", description="Inspect a user's VPS")
