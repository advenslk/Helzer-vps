...
class VPSService:
    ...
    @staticmethod
    def container_name(owner_id: int, name: str) -> str:
        clean = re.sub(r"[^a-z0-9-]+", "-", name.lower()).strip("-") or "vps"
        return f"hx_{owner_id}_{clean[:48]}"[:63]
    ...
    def _dict(vps: VPS) -> dict[str, object]:
        return {"id": vps.id, "owner_id": vps.owner_id, "name": vps.name, "container_id": vps.container_id, "host_port": vps.host_port, "status": vps.status, "cpu_cores": vps.cpu_cores, "ram_mb": vps.ram_mb, "disk_gb": vps.disk_gb, "image": vps.image}
    ...
    async def create_vps(self, owner_id: int, spec: VPSSpec) -> dict[str, object]:
        ...
        if not await asyncio.to_thread(self.docker.can_fit, spec.cpu_cores, spec.ram_mb):
            raise RuntimeError("node does not have enough free CPU/RAM")
        port = self.ports.allocate(); container = None
        try:
            container = await asyncio.to_thread(self.docker.create, name=self.container_name(owner_id, spec.name), image=spec.image, cpu_cores=spec.cpu_cores, ram_mb=spec.ram_mb, host_port=port)
            ...
            if container is not None:
                try: await asyncio.to_thread(self.docker.delete, container.id)
                except Exception: pass
            raise

    async def get(self, vps_id: int):
    ...
    async def list_for_user(self, owner_id: int):
        if not self.sessions: raise RuntimeError("VPS service is not configured")
        async with self.sessions() as session:
            result = await session.scalars(select(VPS).where(VPS.owner_id == owner_id).order_by(VPS.id.desc())); return [self._dict(v) for v in result]

    async def all_vps(self):
        if not self.sessions: raise RuntimeError("VPS service is not configured")
        async with self.sessions() as session:
            result = await session.scalars(select(VPS).order_by(VPS.id)); return [self._dict(v) for v in result]

    async def action(self, vps_id: int, action: str):
    ...
    async def rename(self, vps_id: int, owner_id: int, name: str):
    ...
    async def stats(self, vps_id: int):
        vps = await self.get(vps_id)
        if not vps or not vps.get("container_id") or not self.docker: raise ValueError("VPS not found")
        return await asyncio.to_thread(self.docker.stats, str(vps["container_id"]))

    async def reinstall(self, vps_id: int, owner_id: int, image: str | None = None):
        if not self.sessions or not self.docker: raise RuntimeError("VPS service is not configured")
        vps = await self.get(vps_id)
        if not vps or vps["owner_id"] != owner_id: raise ValueError("VPS not found")
        target = image or str(vps["image"])
        if target not in self.ALLOWED_IMAGES: raise ValueError("Docker image is not allowed")
        old_id = str(vps["container_id"])
        await asyncio.to_thread(self.docker.delete, old_id)
        container = await asyncio.to_thread(self.docker.create, name=self.container_name(owner_id, str(vps["name"])), image=target, cpu_cores=int(vps["cpu_cores"]), ram_mb=int(vps["ram_mb"]), host_port=int(vps["host_port"]))
        try: await asyncio.to_thread(container.start)
        except Exception:
            try: await asyncio.to_thread(self.docker.delete, container.id)
            except Exception: pass
            raise
        async with self.sessions() as session:
            row = await session.get(VPS, vps_id); row.container_id = container.id; row.image = target; row.status = "running"; await session.commit(); return self._dict(row)
